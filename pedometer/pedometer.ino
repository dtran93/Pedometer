...
//   Serial.println("hererre");
//   Serial.println(runningSum, 7);
//   Serial.println(posteriorSit, 7);
//   Serial.println(posteriorWalk, 7);
//   Serial.println(posteriorRun, 7);
   if (counterS % 35 == 0) {
     calOld = cal;
...
void drawBar(int left, int top, int color) {
  if (top < 0) {
    tft.fillRect(left, originLine + top, width, top * -1, color);
  }else {
    tft.fillRect(left, originLine, width, top, color);
  }
}
...
// for at least the counter duration
  ...
  delay (500);  // Add delay for accumulation of samples
